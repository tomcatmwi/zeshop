...
export class FormValidators {
    ...
    static multiple_email(control: FormControl) {
        ...
        const regex = /^[A-Z0-9_'%=+!`#~$*?^{}&|-]+([\.][A-Z0-9_'%=+!`#~$*?^{}&|-]+)*@[A-Z0-9-]+(\.[A-Z0-9-]+)+$/i;
        let value = control.value.replace(/ /ig, '');
...
//  -----------------------------------------------------------------------------------------------------
    ...
    static validatePhone(phoneDistrictKey: string, phoneNumberKey) {
        return (group: FormGroup): {[key: string]: any} => {
            ...
            const phoneNumber = group.controls[phoneNumberKey];
    ...
    static validatePassword(password1Key: string, password2Key: string) {
        return (group: FormGroup): {[key: string]: any} => {
            const password_1 = group.controls[password1Key];
            const password_2 = group.controls[password2Key];
